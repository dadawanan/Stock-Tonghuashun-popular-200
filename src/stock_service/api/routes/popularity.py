...
async def api_popularity_compare_latest(
    ...
    try:
        snapshots = await get_latest_distinct_snapshots(session, limit=2)
        if len(snapshots) < 2:
            raise HTTPException(status_code=400, detail="至少需要两次榜单快照才能比较")
        current_snapshot_time, current_rows = snapshots[0]
        previous_snapshot_time, previous_rows = snapshots[1]
        return ApiResponse(
            data={
