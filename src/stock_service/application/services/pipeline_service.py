...
async def run_all_pipeline(session: AsyncSession) -> dict:
    fetch_popularity_result = await run_popularity_pipeline(session)
    ...
    fetch_result = await run_fetch_pipeline_for_rows(session, new_entries, run_type="fetch", source="ths_new_entries")
    stock_codes = [row["stock_code"] for row in new_entries]
    results, meta = await run_analysis(session, stock_codes=stock_codes)
    count = await store_analysis_results(session, results, meta, run_type="run_all", source="rule")
    return {
        "popularity": fetch_popularity_result,
